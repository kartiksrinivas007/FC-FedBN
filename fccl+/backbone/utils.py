...
from BN_models import MLP, ConvNet, LeNet, AlexNet, AlexNetBN, VGG11, VGG11BN, ResNet18, ResNet18BN_AP, ResNet18BN, AE
...
def get_dataset(dataset, data_path):
    ...
    #     class_names = [0,1]
        ...
        im_size = (224, 224)
        num_classes = 2
        # mean, std = compute_img_mean_std(os.path.join(data_path, 'train'), im_size[0])
        mean, std = [0.7602, 0.5367, 0.5379], [0.0951, 0.1192, 0.1331]
        transform = transforms.Compose([transforms.Resize(im_size), transforms.ToTensor(), transforms.Normalize(mean=[0.7602, 0.5367, 0.5379], std=[0.0951, 0.1192, 0.1331])])
        dst_train = datasets.ImageFolder(root=os.path.join(data_path, 'train'), transform=transform) # no augmentation
        # mean, std = compute_img_mean_std(os.path.join(data_path, 'test'), im_size[0])
        transform = transforms.Compose([transforms.Resize(im_size), transforms.ToTensor(), transforms.Normalize(mean=[0.7543, 0.5393, 0.5420], std=[0.0989, 0.1222, 0.1364])])
...
def get_network(model, channel, num_classes, im_size=(32, 32)):
    ...
    if model == 'MLP':
        ...
        net = AlexNetBN(channel=channel, num_classes=num_classes, im_size=im_size)
    elif model == 'VGG11':
        net = VGG11( channel=channel, num_classes=num_classes, im_size=im_size)
    elif model == 'VGG11BN':
        net = VGG11BN(channel=channel, num_classes=num_classes, im_size=im_size)
    elif model == 'ResNet18':
        net = ResNet18(channel=channel, num_classes=num_classes, im_size=im_size)
    elif model == 'ResNet18BN_AP':
        net = ResNet18BN_AP(channel=channel, num_classes=num_classes, im_size=im_size)
    elif model == 'ResNet18BN':
        net = ResNet18BN(channel=channel, num_classes=num_classes, im_size=im_size)
...
def epoch_ae(mode, dataloader, net, optimizer, criterion, args, aug=False):
    ...
    for i_batch, datum in enumerate(dataloader):
        ...
        if i_batch == 0 and mode!='train':
            save_image(output, f'ae_{args.dataset}example.png') # Trying normalize = True/False may get better visual effects.
...
def rand_scale(x, param):
    ...
    if param.Siamese and theta.size(0) != 1: # Siamese augmentation:
        theta[:] = theta[0]
...
def rand_flip(x, param):
    ...
    if param.Siamese and randf.size(0) != 1: # Siamese augmentation:
        randf[:] = randf[0]
...
def rand_brightness(x, param):
    ...
    if param.Siamese and randb.size(0) != 1:  # Siamese augmentation:
        randb[:] = randb[0]
    x = x + (randb - 0.5)*ratio
    return x
...
def rand_saturation(x, param):
    ...
    if param.Siamese and rands.size(0) != 1:  # Siamese augmentation:
        rands[:] = rands[0]
...
def rand_contrast(x, param):
    ...
    if param.Siamese and randc.size(0) != 1:  # Siamese augmentation:
        randc[:] = randc[0]
...
def rand_crop(x, param):
    ...
    if param.Siamese and translation_x.size(0) != 1:  # Siamese augmentation:
...
def rand_cutout(x, param):
    ...
    if param.Siamese and offset_x.size(0) != 1:  # Siamese augmentation:
        offset_x[:] = offset_x[0]
        offset_y[:] = offset_y[0]
    grid_batch, grid_x, grid_y = torch.meshgrid(
        torch.arange(x.size(0), dtype=torch.long, device=x.device),
