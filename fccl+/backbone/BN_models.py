...
class AlexNetBN(nn.Module):
    ...
    def forward(self, x):
        x = self.features(x)
        feature = x.view(x.size(0), -1) # flattened here
        out = self.fc(feature)
...
class VGG(nn.Module):
    ...
    def forward(self, x):
        ...
        feature = x.view(x.size(0), -1)
        out = self.classifier(feature)
        return out # modified, feature is not returned here
...
def VGG11BN(channel, num_classes, im_size):
    return VGG('VGG11', channel, num_classes, im_size=im_size, norm='batchnorm')
...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, channel=3, num_classes=10, im_size=(28, 28), norm='instancenorm'):
        ...
        self.layer3 = self._make_layer(block, 256, num_blocks[2], stride=2)
        self.layer4 = self._make_layer(block, 512, num_blocks[3], stride=2)
        if im_size[0] == 28 or im_size[0] == 32:
...
def ResNet18BN(channel, num_classes, im_size):
    return ResNet(BasicBlock, [2,2,2,2], channel=channel, num_classes=num_classes, im_size=im_size, norm='batchnorm')
...
class Encoder(nn.Module):
    ...
    def __init__(self, channel, depths=[8, 16, 32, 64], encoded_space_dim=64):
        super().__init__()

        first_pad = 3 if channel == 1 else 1

        
        ### Convolutional section
        layers = []
        layers.append(nn.Conv2d(channel, 8, 3, stride=2, padding=first_pad))
        layers.append(nn.ReLU(True))
        for i in range(len(depths)-1):
            layers.append(nn.Conv2d(depths[i], depths[i+1], 3))
            layers.append(nn.BatchNorm2d(depths[i+1]))
            layers.append(nn.ReLU(True))
        self.encoder_cnn = nn.Sequential(*layers)
            
        # self.encoder_cnn = nn.Sequential(
        #     nn.Conv2d(channel, 8, 3, stride=2, padding=1),
        #     nn.ReLU(True),
...
class Decoder(nn.Module):
    ...
    def __init__(self, channel, depths=[64, 32, 16, 8], encoded_space_dim=64):
        super().__init__()
        ...
        self.decoder_lin = nn.Sequential(
            ...
            nn.Linear(128, 1024),
            nn.ReLU(True),
            nn.Linear(1024, 10*10*depths[0]),
            nn.ReLU(True)
        )

        self.unflatten = nn.Unflatten(dim=1, 
        unflattened_size=(depths[0], 10, 10))

        layers = []
        for i in range(len(depths)-1):
            layers.append(nn.ConvTranspose2d(depths[i], depths[i+1], 3))
            layers.append(nn.BatchNorm2d(depths[i+1]))
            layers.append(nn.ReLU(True))
        layers.append(nn.ConvTranspose2d(8, channel, 3, stride=2, padding=first_pad, output_padding=1))
        ...
        #     nn.BatchNorm2d(8),
        #     nn.ReLU(True),
        #     nn.ConvTranspose2d(8, channel, 3, stride=2, 
        #     padding=1, output_padding=1)
        # )
...
class AE(nn.Module):
    ...
    def __init__(self, channel, encoded_space_dim=64, depths=[8, 16, 32, 64]):
        super().__init__()

        encoded_space_dim = 64

        ### endocer
        self.encoder = Encoder(channel, depths, encoded_space_dim)

        ### decoder
        self.decoder = Decoder(channel, depths[::-1], encoded_space_dim)
...
# def AlexNetBNNetworkWrapper(model_name, channels, num_classes, image_size):
